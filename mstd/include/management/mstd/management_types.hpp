...
namespace mstd {
	...
	template<class T>
	struct is_signed_integral : std::bool_constant<is_signed_integral_v<T> > {};
	template<class T>
	constexpr bool is_unsigned_integral_v = std::is_integral_v<T> && std::is_unsigned_v<T>;

	template<class T>
	struct is_unsigned_integral : std::bool_constant<is_unsigned_integral_v<T> > {};
	...
	static _MSTD_CONSTEXPR17 const auto abs_v = utils::abs_impl<A>::value;
		...
		#if _MSTD_HAS_CXX20
	template<auto A, auto B, auto Eps, arithmetic AT = decltype(A), arithmetic BT = decltype(B),
	  floating_point EpsT = decltype(Eps)>
