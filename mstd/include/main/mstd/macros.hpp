...
_MSTD_WARNING("this is only available for c++17 and greater!");
	...
	// define MSTD_ENABLE_FOR_EACH_MACROS or MSTD_ENABLE_ENUMS_MACROS or MSTD_ENABLE_CLONE_FUNC_MACROS to use macros in for each region
		#pragma region FOR_EACH
			...
			#define _FOR_EACH(func, helper, value, ...) _RESCAN(helper(func, value __VA_OPT__(, __VA_ARGS__)))
		...
		#pragma region ENUMS
			...
			#define _ENUM_SIZE_FUNC(name, ...)                            \
				template<class T>                                         \
				inline _MSTD_CONSTEXPR20 size_t size() noexcept;          \
				template<>                                                \
				inline _MSTD_CONSTEXPR20 size_t size<name>() noexcept {   \
			...
			#define _ENUM_FROM_STRING_FUNC(name, case_func, ...)              \
				...
				template<>                                                    \
				inline name from_string<name>(const std::string& s) {         \
					DO_FOR_EACH_WITH_CONST(case_func, name, __VA_ARGS__)      \
			...
			#define _STANDARD_ENUM_STRING_TO_STRING_CASE(enum_name, tuple)                            \
				_INVOKE(_STANDARD_ENUM_STRING_TO_STRING_CASE_HELPER, (enum_name, BREAK_TUPLE(tuple)))
			#define _STANDARD_ENUM_STRING_FROM_STRING_IF_HELPER(enum_name, name, string) \
				if (s == string) return enum_name::name;
			#define _STANDARD_ENUM_STRING_FROM_STRING_IF(enum_name, tuple)                            \
				_INVOKE(_STANDARD_ENUM_STRING_FROM_STRING_IF_HELPER, (enum_name, BREAK_TUPLE(tuple)))
			...
			#define _VALUE_ENUM_STRING_TO_STRING_CASE(enum_name, tuple)                            \
				_INVOKE(_VALUE_ENUM_STRING_TO_STRING_CASE_HELPER, (enum_name, BREAK_TUPLE(tuple)))
			#define _VALUE_ENUM_STRING_FROM_STRING_IF_HELPER(enum_name, name, value, string) \
				if (s == string) return enum_name::name;
			#define _VALUE_ENUM_STRING_FROM_STRING_IF(enum_name, tuple)                            \
				_INVOKE(_VALUE_ENUM_STRING_FROM_STRING_IF_HELPER, (enum_name, BREAK_TUPLE(tuple)))
			...
			#define _VALUE_ENUM_TEMPLATE(type, name, base, ...)                                             \
				_VALUE_ENUM_FUNC_TEMPLATE(type, name, base, _VALUE_ENUM_DECLARATION, _VALUE_ENUM_SIZE_FUNC, \
				  _VALUE_ENUM_TO_STRING_FUNC, _VALUE_ENUM_FROM_STRING_FUNC, __VA_ARGS__)
		...
		#pragma region CLONE_FUNC
			...
			#define DECLARE_CLONE_BASE_FUNC_WITH_DEFINITION_ADVANCED(class_name, base_class_name, ...)     \
				_DECLARE_CLONE_FUNC_WITH_DEFINITION_TEMPLATE(class_name, base_class_name::CloneTo(cloned); \
				  , override __VA_OPT__(, __VA_ARGS__))

			#define DECLARE_CLONE_FUNC_WITH_DEFINITION(class_name, ...)             \
				DECLARE_CLONE_FUNC_WITH_DEFINITION_ADVANCED(class_name __VA_OPT__(, \
				  LIST_DO_FOR_EACH(STANDARD_CLONE, __VA_ARGS__)))
			#define DECLARE_CLONE_BASE_FUNC_WITH_DEFINITION(class_name, base_class_name, ...)  \
				DECLARE_CLONE_BASE_FUNC_WITH_DEFINITION_ADVANCED(class_name,                   \
