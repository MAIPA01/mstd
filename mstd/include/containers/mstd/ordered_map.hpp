...
namespace mstd {
	...
	private:
		...
		_MSTD_CONSTEXPR20 void _update_indexes(size_type const& from) {
	...
	public:
		...
		_MSTD_CONSTEXPR20 ordered_map(ordered_map<Key, T> const& other)		= default;
		_MSTD_CONSTEXPR20 ordered_map(ordered_map<Key, T>&& other) noexcept = default;

		_MSTD_CONSTEXPR20 ordered_map(std::initializer_list<value_type> const& init) {
		...
		_MSTD_CONSTEXPR20 ordered_map& operator=(ordered_map<Key, T> const& other)	   = default;
		_MSTD_CONSTEXPR20 ordered_map& operator=(ordered_map<Key, T>&& other) noexcept = default;

		_MSTD_CONSTEXPR20 T& emplace(const_iterator const& where, Key const& key, T const& value) {
			return insert(where, std::make_pair(key, value));
		}

		_MSTD_CONSTEXPR20 T& emplace_back(Key const& key, T const& value) {
			return emplace(cend(), key, value);
		}

		_MSTD_CONSTEXPR20 T& insert(const_iterator const& where, value_type const& value) {
			...
			size_type whereOffset = std::clamp<size_type>(std::distance(_orderedElements.cbegin(), where), 0,
			  _orderedElements.size() - 1);
			size_type const& elemOffset = _elementsMap.at(value.first);
		...
		_MSTD_CONSTEXPR20 T& insert_back(value_type const& value) { return insert(cend(), value); }
		...
		_MSTD_CONSTEXPR20 void erase(Key const& key) {
		...
		[[nodiscard]] _MSTD_CONSTEXPR20 T& at(Key const& key) {
			mstd_assert(contains(key), "Key '{}' not found", key);
			return _orderedElements.at(_elementsMap.at(key)).second;
		}

		[[nodiscard]] _MSTD_CONSTEXPR20 const T& at(Key const& key) const {
			mstd_assert(contains(key), "Key '{}' not found", key);
			return _orderedElements.at(_elementsMap.at(key)).second;
		}
		...
		[[nodiscard]] _MSTD_CONSTEXPR20 bool contains(Key const& key) const {
		#if _MSTD_HAS_CXX20
		...
		[[nodiscard]] _MSTD_CONSTEXPR20 iterator find(Key const& key) {
			auto it = _elementsMap.find(key);
			return it != _elementsMap.end() ? std::next(_orderedElements.begin(), it->second) :
											  _orderedElements.end();
		}

		[[nodiscard]] _MSTD_CONSTEXPR20 const_iterator find(Key const& key) const {
			auto it = _elementsMap.find(key);
			return it != _elementsMap.end() ? std::next(_orderedElements.begin(), it->second) :
											  _orderedElements.end();
		}
		...
		[[nodiscard]] _MSTD_CONSTEXPR20 const_reverse_iterator rbegin() const {
			return _orderedElements.crbegin();
		}

		[[nodiscard]] _MSTD_CONSTEXPR20 const_reverse_iterator rend() const {
			return _orderedElements.crend();
		}

		[[nodiscard]] _MSTD_CONSTEXPR20 const_reverse_iterator crbegin() const {
			return _orderedElements.crbegin();
		}

		[[nodiscard]] _MSTD_CONSTEXPR20 const_reverse_iterator crend() const {
			return _orderedElements.crend();
		}

		[[nodiscard]] _MSTD_CONSTEXPR20 T& operator[](Key const& key) {
				if (!contains(key)) { return emplace_back(key, T()); }
			return at(key);
		}

		[[nodiscard]] _MSTD_CONSTEXPR20 const T& operator[](Key const& key) const { return at(key); }

		[[nodiscard]] _MSTD_CONSTEXPR20 bool operator==(ordered_map<Key, T> const& other) const {
