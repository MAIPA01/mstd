...
namespace mstd::test {
	TEST(EVENTS_EVENT_HANDLER, BASIC_INVOKE) {
		mstd::method_event_handler handler;
		int call_count = 0;
	...
	TEST(EVENTS_EVENT_HANDLER, ARGUMENT_PASSING) {
		mstd::event_handler<int, std::string> handler;
		int received_int		 = 0;
		std::string received_str = "";

		handler.add_callback([&](const int i, const std::string& s) {
			received_int = i;
			received_str = s;
		});
	...
	TEST(EVENTS_EVENT_HANDLER, REENTRANCY_SAFETY_SELF_REMOVAL) {
		...
		handler.add_callback([&] { second_called = true; });
