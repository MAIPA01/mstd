...
namespace mstd::test {
    ...
    TEST(FUNCTIONS_CoreTest, FunctionPredicates) {
        ...
        // is_function_v: true dla wskaźników (wolne/metody), false dla obiektów wywoływalnych (funktory/std::function)
        EXPECT_TRUE(is_function_v<GlobalFuncPtr>);
        EXPECT_TRUE(is_function_v<MemberFuncPtr>);

        EXPECT_FALSE(is_function_v<LambdaType>);  // Bo to funktor
        EXPECT_FALSE(is_function_v<StdFuncType>); // Bo to std::function
        EXPECT_FALSE(is_function_v<int>);         // Bo nie jest wywoływalne

        // is_action_v: funkcja (wskaźnik) zwracająca void
        EXPECT_TRUE(is_action_v<decltype(&global_action)>);
        EXPECT_TRUE(is_action_v<decltype(&TestClass::void_method)>);
        EXPECT_FALSE(is_action_v<GlobalFuncPtr>); // Bo zwraca int

        // is_method_v: akcja (void) bez argumentów
        EXPECT_TRUE(is_method_v<decltype(&TestClass::void_method)>);
        EXPECT_TRUE(is_method_v<decltype(&TestClass::static_method)>);
        EXPECT_FALSE(is_method_v<decltype(&global_action)>); // Bo ma argument int
    }
