...
namespace mstd {
	...
	struct string_type_info<T, std::enable_if_t<
		...
		static _MSTD_CONSTEXPR20 size_t size(const T&) { return 1; }
	};
	...
	struct string_type_info<T[N], std::enable_if_t<
		...
		std::is_same_v<std::remove_cv_t<T>, signed char> ||
		std::is_same_v<std::remove_cv_t<T>, unsigned char>>> {
		static _MSTD_CONSTEXPR20 size_t size(const T(&)[N]) { return N - 1; }
	};
	...
	struct string_type_info<T, std::enable_if_t<
		...
		static _MSTD_CONSTEXPR20 size_t size(T s) {
	...
	struct string_type_info<T, std::enable_if_t<std::is_same_v<std::remove_cv_t<T>, std::string>>> {
		static _MSTD_CONSTEXPR20 size_t size(const std::string& s) { return s.size(); }
	};
	
	template<class _String>
	static inline _MSTD_CONSTEXPR20 size_t string_size(_String&& s) {
